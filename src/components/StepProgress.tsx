...
export function StepProgress({ currentStep, onNavigate }: Props) {
  ...
  return (
    <div className="bg-white/70 backdrop-blur-2xl border-b border-slate-100/80 sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3.5">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2.5 font-display font-bold text-lg text-slate-900 hover:text-brand-600 transition-colors duration-200 shrink-0"
          >
            ...
            {FLOW.map((s, i) => {
              ...
              return (
                <React.Fragment key={s.key}>
                  ...
                  >
                    <div className={`w-5.5 h-5.5 rounded-full flex items-center justify-center text-[10px] font-bold transition-all duration-300 ${
                      isDone
                        ? 'bg-gradient-to-br from-emerald-400 to-emerald-500 text-white shadow-sm shadow-emerald-500/20'
                        : isCurrent
                  ...
                  {i < FLOW.length - 1 && (
                    <div className={`w-5 sm:w-8 h-0.5 rounded-full transition-colors duration-500 ${
